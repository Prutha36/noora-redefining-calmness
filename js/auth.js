/* ============================================================
   ...
   async function onSignedIn() {
     updateNavUser();
     await loadWishlist();
     // merge any guest wishlist items into supabase
     var guest = getGuestWishlist();
     for (var pid of guest) {
       if (!wishlistIds.has(pid)) {
         await sb.from('wishlist').insert({ user_id: currentUser.id, product_id: pid }).then(function(){});
         wishlistIds.add(pid);
       }
     }
     localStorage.removeItem('noora-wishlist');
   ...
   function toggleUserMenu() {
     var existing = document.getElementById('userDropMenu');
     if (existing) { existing.remove(); return; }
     var menu = document.createElement('div');
     menu.id = 'userDropMenu'; menu.className = 'user-drop-menu';
     menu.innerHTML =
   ...
   async function doSignIn() {
     ...
     if (result.error) {
       ...
       setTimeout(function() {
         ...
         setTimeout(function() {
           var ei = document.getElementById('authEmail'); if (ei) ei.value = ev;
           var er = document.getElementById('authError'); if (er) er.textContent = '';
         }, 50);
       }, 2000);
       return;
     }
   ...
   async function doSignUp() {
     ...
     if (result.data && result.data.user) {
       await sb.from('profiles').upsert({ id: result.data.user.id, full_name: name, email: email });
     }
   ...
   async function signInWithGoogle() {
     ...
     if (result.error) setAuthError('Google sign in failed. Try email instead.');
   }
   ...
   async function loadWishlist() {
     ...
     var result = await sb.from('wishlist').select('product_id').eq('user_id', currentUser.id);
     wishlistIds = new Set((result.data || []).map(function(r){ return r.product_id; }));
   }
   ...
   async function toggleWishlist(pid, btn) {
     if (currentUser) {
       ...
       if (wishlistIds.has(pid)) {
         ...
         btn.classList.remove('wishlisted'); btn.textContent = '🤍';
       } else {
         await sb.from('wishlist').insert({ user_id: currentUser.id, product_id: pid });
         wishlistIds.add(pid);
         btn.classList.add('wishlisted'); btn.textContent = '❤️';
       }
     } else {
   ...
   async function openOrderHistory() {
     var m = document.getElementById('userDropMenu'); if (m) m.remove();
     var result = await sb.from('orders').select('*').eq('user_id', currentUser.id).order('created_at', { ascending: false });
     ...
     panel.innerHTML = '<div class="side-panel-overlay" onclick="document.getElementById(\'orderHistoryPanel\').remove()"></div>' +
       '<div class="side-panel">' +
         '<div class="side-panel-header"><h3>My Orders</h3><button onclick="document.getElementById(\'orderHistoryPanel\').remove()">✕</button></div>' +
         '<div class="side-panel-body">' +
         (!orders || !orders.length ? '<p class="panel-empty">No orders yet 🌸</p>' :
           orders.map(function(o) {
             return '<div class="order-card">' +
               '<div class="order-card-top"><span class="order-date">' + new Date(o.created_at).toLocaleDateString('en-IN',{day:'numeric',month:'short',year:'numeric'}) + '</span>' +
               '<span class="order-status ' + o.status + '">' + o.status + '</span></div>' +
               (o.items||[]).map(function(i){ return '<div class="order-item">• ' + i.name + ' × ' + i.qty + '</div>'; }).join('') +
               '<div class="order-total">Total: ₹' + (o.total ? o.total.toFixed(2) : '—') + '</div>' +
             '</div>';
           }).join('')) +
         '</div></div>';
   ...
   async function openSavedAddress() {
     var m = document.getElementById('userDropMenu'); if (m) m.remove();
     var result = await sb.from('addresses').select('*').eq('user_id', currentUser.id).order('is_default', { ascending: false });
     ...
     panel.innerHTML = '<div class="side-panel-overlay" onclick="document.getElementById(\'addressPanel\').remove()"></div>' +
       '<div class="side-panel">' +
         '<div class="side-panel-header"><h3>Saved Addresses</h3><button onclick="document.getElementById(\'addressPanel\').remove()">✕</button></div>' +
         '<div class="side-panel-body">' +
         (!addrs || !addrs.length ? '<p class="panel-empty">No saved addresses yet 🌸</p>' :
           addrs.map(function(a){
             return '<div class="address-card ' + (a.is_default?'default':'') + '">' +
               ...
               '<div class="address-phone">' + a.phone + '</div></div>';
           }).join('')) +
   ...
   function openAddAddressForm() {
     var body = document.querySelector('#addressPanel .side-panel-body');
     if (!body) return;
     body.innerHTML =
   ...
   async function saveAddress() {
     var addr = {
       ...
       full_name:    document.getElementById('addrName') ? document.getElementById('addrName').value.trim() : '',
       phone:        document.getElementById('addrPhone') ? document.getElementById('addrPhone').value.trim() : '',
       address_line: document.getElementById('addrLine') ? document.getElementById('addrLine').value.trim() : '',
       city:         document.getElementById('addrCity') ? document.getElementById('addrCity').value.trim() : '',
       state:        document.getElementById('addrState') ? document.getElementById('addrState').value.trim() : '',
       pincode:      document.getElementById('addrPin') ? document.getElementById('addrPin').value.trim() : '',
       is_default:   true,
     };
     var result = await sb.from('addresses').insert(addr);
     if (result.error) { showToast('Error saving address'); return; }
   ...
   async function openWishlistPanel() {
     var m = document.getElementById('userDropMenu'); if (m) m.remove();
     var ids = currentUser ? wishlistIds : getGuestWishlist();
     var wishProducts = products.filter(function(p){ return ids.has(p.id); });
     ...
     panel.innerHTML = '<div class="side-panel-overlay" onclick="document.getElementById(\'wishlistPanel\').remove()"></div>' +
       '<div class="side-panel">' +
         '<div class="side-panel-header"><h3>My Wishlist 🤍</h3><button onclick="document.getElementById(\'wishlistPanel\').remove()">✕</button></div>' +
         '<div class="side-panel-body">' +
         (!wishProducts.length ? '<p class="panel-empty">Your wishlist is empty 🌸<br><br><a href="shop.html" style="color:var(--rose);text-decoration:none">Browse candles →</a></p>' :
