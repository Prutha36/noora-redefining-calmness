...
function saveGuestWishlist(s) { localStorage.setItem('noora-wishlist', JSON.stringify([...s])); }

function updateWishlistCount() {
  var count = currentUser ? wishlistIds.size : getGuestWishlist().size;
  var el = document.getElementById('wishlistCount'); if (!el) return;
...
async function onSignedIn() {
  updateNavUser(); await loadWishlist();
  var guest = getGuestWishlist();
  var pids = [...guest];
  for (var i = 0; i < pids.length; i++) {
    if (!wishlistIds.has(pids[i])) {
      await sb.from('wishlist').insert({ user_id: currentUser.id, product_id: pids[i] });
      wishlistIds.add(pids[i]);
    }
  }
  if (guest.size) localStorage.removeItem('noora-wishlist');
  renderWishlistHearts(); updateWishlistCount(); updateCartUI();
}
...
function updateNavUser() {
  ...
  if (currentUser) {
    var nm = (currentUser.user_metadata && currentUser.user_metadata.full_name) ? currentUser.user_metadata.full_name : currentUser.email;
    var ini = nm.split(' ').map(function(w){ return w[0]||''; }).join('').toUpperCase().slice(0,2);
    btn.innerHTML = '<div class="nav-avatar" id="navAvatar" onclick="toggleUserMenu()">' + ini + '</div>';
  } else {
...
function toggleUserMenu() {
  ...
  var m = document.createElement('div'); m.id='userDropMenu'; m.className='user-drop-menu';
  m.innerHTML = '<div class="user-drop-email">'+currentUser.email+'</div>' +
  ...
  document.getElementById('navAuthBtn').appendChild(m);
...
function switchAuthTab(tab) {
  document.querySelectorAll('.auth-tab').forEach(function(t){ t.classList.toggle('active', t.textContent.toLowerCase().replace(/\s/g,'')===tab); });
  renderAuthTab(tab);
}
function renderAuthTab(tab) {
  var el = document.getElementById('authTabContent'); if (!el) return;
  if (tab==='signin') {
    el.innerHTML = '<div class="auth-field"><label>Email</label><input type="email" id="authEmail" placeholder="you@email.com"/></div>' +
    ...
    el.innerHTML = '<div class="auth-field"><label>Full Name</label><input type="text" id="authName" placeholder="Noora Al-Farsi"/></div>' +
...
function setAuthError(msg) { var e=document.getElementById('authError'); if(e) e.textContent=msg; }
...
async function doSignIn() {
  ...
  if (res.error) { setAuthError('No account found or incorrect password.'); setTimeout(function(){ var ev=document.getElementById('authEmail')?document.getElementById('authEmail').value:''; switchAuthTab('signup'); setTimeout(function(){ var ei=document.getElementById('authEmail'); if(ei) ei.value=ev; var er=document.getElementById('authError'); if(er) er.textContent=''; },50); },2000); return; }
...
async function signInWithGoogle() {
  ...
  if (res.error) setAuthError('Google sign in failed. Try email instead.');
}
...
function renderWishlistHearts() {
  ...
  document.querySelectorAll('.wishlist-btn').forEach(function(btn) {
    var pid=parseInt(btn.dataset.pid); var on=ids.has(pid);
    btn.textContent = on?'❤️':'🤍'; btn.classList.toggle('wishlisted',on);
  });
  updateWishlistCount();
}
...
async function openOrderHistory() {
  var dm=document.getElementById('userDropMenu'); if(dm) dm.remove();
  ...
  panel.innerHTML='<div class="side-panel-overlay" onclick="document.getElementById(\'orderHistoryPanel\').remove()"></div><div class="side-panel"><div class="side-panel-header"><h3>My Orders</h3><button onclick="document.getElementById(\'orderHistoryPanel\').remove()">✕</button></div><div class="side-panel-body">'+
    (!res.data||!res.data.length?'<p class="panel-empty">No orders yet 🌸</p>':res.data.map(function(o){ return '<div class="order-card"><div class="order-card-top"><span class="order-date">'+new Date(o.created_at).toLocaleDateString('en-IN',{day:'numeric',month:'short',year:'numeric'})+'</span><span class="order-status '+(o.status||'pending')+'">'+(o.status||'pending')+'</span></div>'+(o.items||[]).map(function(i){ return '<div class="order-item">• '+i.name+' × '+i.qty+'</div>'; }).join('')+'<div class="order-total">Total: ₹'+(o.total?o.total.toFixed(2):'—')+'</div></div>'; }).join(''))+
    '</div></div>';
...
async function openSavedAddress() {
  ...
  panel.innerHTML='<div class="side-panel-overlay" onclick="document.getElementById(\'addressPanel\').remove()"></div><div class="side-panel"><div class="side-panel-header"><h3>Saved Addresses</h3><button onclick="document.getElementById(\'addressPanel\').remove()">✕</button></div><div class="side-panel-body">'+
    (!res.data||!res.data.length?'<p class="panel-empty">No saved addresses yet 🌸</p>':res.data.map(function(a){ return '<div class="address-card '+(a.is_default?'default':'')+'">'+( a.is_default?'<span class="default-badge">Default</span>':'')+
    '<div class="address-name">'+a.full_name+'</div><div class="address-line">'+a.address_line+', '+a.city+', '+a.state+' — '+a.pincode+'</div><div class="address-phone">'+a.phone+'</div></div>'; }).join(''))+
    '<button class="add-address-btn" onclick="openAddAddressForm()">+ Add New Address</button></div></div>';
  document.body.appendChild(panel);
}

function openAddAddressForm() {
  var body=document.querySelector('#addressPanel .side-panel-body'); if(!body) return;
  body.innerHTML='<div class="auth-field"><label>Full Name</label><input type="text" id="addrName" placeholder="Noora Al-Farsi"/></div><div class="auth-field"><label>Phone</label><input type="tel" id="addrPhone" placeholder="+91 98765 43210"/></div><div class="auth-field"><label>Address</label><input type="text" id="addrLine" placeholder="House no, Street, Area"/></div><div class="auth-field"><label>City</label><input type="text" id="addrCity" placeholder="Mumbai"/></div><div class="auth-field"><label>State</label><input type="text" id="addrState" placeholder="Maharashtra"/></div><div class="auth-field"><label>Pincode</label><input type="text" id="addrPin" placeholder="400001"/></div><button class="auth-submit-btn" onclick="saveAddress()">Save Address ✿</button>';
}

async function saveAddress() {
  var f=function(id){ return document.getElementById(id)?document.getElementById(id).value.trim():''; };
  var addr={user_id:currentUser.id,full_name:f('addrName'),phone:f('addrPhone'),address_line:f('addrLine'),city:f('addrCity'),state:f('addrState'),pincode:f('addrPin'),is_default:true};
  var res=await sb.from('addresses').insert(addr);
  if(res.error){showToast('Error saving address');return;}
  showToast('Address saved! 🌸'); var p=document.getElementById('addressPanel'); if(p) p.remove();
}

/* ── WISHLIST PANEL — product images, no emoji overlap ── */
async function openWishlistPanel() {
  var dm=document.getElementById('userDropMenu'); if(dm) dm.remove();
  var ids = currentUser ? wishlistIds : getGuestWishlist();
  var wishPs = (typeof products!=='undefined') ? products.filter(function(p){ return ids.has(p.id); }) : [];

  function wlItem(p) {
    var minPrice = Math.min.apply(null, p.fragrances.map(function(f){ return f.price; }));
    return '<div class="wishlist-item">' +
      '<div class="wl-img-wrap">' +
        '<img class="wl-img" src="' + p.image + '" alt="' + p.name + '"' +
          ' onerror="this.style.display=\'none\';this.nextElementSibling.style.display=\'flex\'">' +
        '<div class="wl-img-fallback" style="display:none">' + p.emoji + '</div>' +
      '</div>' +
      '<div class="wishlist-item-info">' +
        '<div class="wishlist-item-name">' + p.name + '</div>' +
        '<div class="wishlist-item-price">from ₹' + minPrice + '</div>' +
  ...
  panel.innerHTML='<div class="side-panel-overlay" onclick="document.getElementById(\'wishlistPanel\').remove()"></div>' +
    '<div class="side-panel">' +
    '<div class="side-panel-header"><h3>Wishlist 🤍 ('+wishPs.length+')</h3><button onclick="document.getElementById(\'wishlistPanel\').remove()">✕</button></div>' +
    '<div class="side-panel-body">' +
    (!wishPs.length ? '<p class="panel-empty">Your wishlist is empty 🌸<br><br><a href="shop.html" style="color:var(--rose);text-decoration:none;font-style:normal">Browse candles →</a></p>' :
      wishPs.map(wlItem).join('')) +
    '</div></div>';
  document.body.appendChild(panel);
}

async function removeFromWishlistPanel(pid,btn) {
  if (currentUser) { await sb.from('wishlist').delete().eq('user_id',currentUser.id).eq('product_id',pid); wishlistIds.delete(pid); }
  else { var gw=getGuestWishlist(); gw.delete(pid); saveGuestWishlist(gw); }
